...
template <typename T> void nn<T>::mean_absolute_error(){
    const ui32 last_layer_index = nn_shape.size() - 1;
    err = 0.0;
...
template <typename T> void nn<T>::mean_bias_error(){
    const ui32 last_layer_index = nn_shape.size() - 1;
    err = 0.0;
...
template <typename T> void nn<T>::cross_entropy_error(){
    const ui32 last_layer_index = nn_shape.size() - 1;
    err = 0.0;
...
template <typename T> void nn<T>::hinge_error(){
    const ui32 last_layer_index = nn_shape.size() - 1;
    err = 0.0;
    /*  HINGE ERROR METHOD  */
    for(ui32 i = 0U; i < nn_shape[last_layer_index; ++i]){
...
template <typename T> void nn<T>::compute_error(){
